...
нарушению целостности. Это особенно важно для реляционных БД с ограничениями (UNIQUE, NOT NULL, CHECK). 
1) Очистка данных от дубликатов (по productName для продуктов и username для клиентов):
- Аномалии: Дубликаты записей. Это нарушает UNIQUE-конstraints в БД, приводит к ошибкам вставки и дублированию данных в 
отчетах.
...
- Аномалии: NULL-значения в categoryName (например, продукт без категории). Это может вызвать ошибки в FK-связях 
